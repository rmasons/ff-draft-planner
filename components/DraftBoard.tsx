...
export default function DraftBoard() {
  ...
  const rows = useMemo(() => {
    ...
    const sorted = [...list].sort((a, b) =>
      filter === "ALL" ? a.overallRank - b.overallRank : a.posRank - b.posRank
    );
    return hideDrafted ? sorted.filter((p) => !draftedSet.has(p.id)) : sorted;
  }, [ranked, filter, query, hideDrafted, draftedSet]);
  ...
  const showTierDividers = filter !== "ALL" && !query.trim();
  // Replacement line only makes sense within a single position, undrafted-first.
  const replRank =
    filter !== "ALL" && !query.trim() && baselines ? baselines[filter].rank : null;
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((p, i) => {
                  ...
                  // Positive = our rank beats ADP (drafted later = steal/value)
                  // Negative = ADP beats our rank (drafted earlier = reach)
                  const value = adpDisplay !== null ? Math.round(adpDisplay - p.overallRank) : null;
                  return (
                    <Row
                      key={p.id}
                      p={p}
